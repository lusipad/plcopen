...
```
  ...
  第 1 层：完整 IEC 61131-3 标准功能块库（定时器 / 计数器 / 逻辑 / 数学）
                ↑
  第 0 层：PLCopen Part 1&2 运动控制库【当前在做】
```

### 第 0 层：PLCopen Part 1&2 运动控制库（当前）

**现状**：70% 完成（单轴运动块、状态机、梯形曲线）
...
| 能力 | 解锁条件（任一满足即启动评估） |
|------|--------------------------------|
| S 曲线（Jerk 受限） | v0.2 稳定后 OR 1 个用户 issue |
| MC_Home 完整实现 | v0.2 稳定后（当前 75%） |
| 多轴凸轮/齿轮 | 1 个用户 issue + 有测试硬件 |
| Python 绑定 | ≥3 个"Python 能调吗"询问 |
| IEC 61131-3 标准功能块库 | v0.3 后 OR 用户明确需要 |
| ST 编译器 MVP | ≥5 个用户要求 + 有合作者 |
| Modbus 适配器 | 有正在使用库的项目提集成需求 |
| OPC UA | 1 个工业客户深度集成承诺 |
| LD / FBD 编辑器 | 库被 ≥10 个项目引用 |
| SIL 认证 | 有客户愿意承担认证成本 |
| 分布式 PLC | 多个大规模部署用户 |

...
3. **P0 主线复绿（已完成）**：`17932de` 已修复测试辅助函数无谓按值复制
...
4. **P1 CI 时长恢复（已完成）**：`0195739` 已把 11 项 fuzz CTest 统一移到原生
...
5. **A1 浮点数值语义合同（已完成）**：`805a363` 已向 header-only consumer
...
6. **无外部前置的软件队列**：v0.20.0、A2 T30 WCET、G1 治理与 X5
   ...
   MuJoCo/Rerun 闭环孪生也已完成（KB-090）：默认依赖保持不变，
...
7. **维护者授权的软件队列（已完成）**：2026-07-24 已明确按
   **D3 Scope → D1 ST LSP → H1 同步关节流 → T2b 大模型孪生 →
   ...
   主线全绿。T2b（KB-093）也已由
   [PR #32](https://github.com/lusipad/plcopen/pull/32) 合入：最小
   `JointStreamSim` q/dq facade、仓库自有七关节 primitive 模型、
   100 Hz→1 kHz H1 升频、2,000 tick/200 帧 MuJoCo/Rerun headless
   闭环与 T2a 回归均通过，最大末误差 `0.01133902048 rad`；合入后的
   [Windows](https://github.com/lusipad/plcopen/actions/runs/30136405358)、
...
9. **完成面守护**：PLCopen/Beckhoff C0→C6 与 ST L0→L7、L∀ 已完成；只

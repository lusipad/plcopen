...
| 4c | Y4 solve_fixed_time 一等原语（评审三关键，紧跟 Y2） | 同步 + cycle-exact 量化（删 KB-050 尾段补丁）+ 流追赶汇合同一求解；终态 ≤1e-9（T43） | **求解器已交付**（KB-056：3 候选 + 9 固定 + 200 fuzz）；集成（删 KB-050 补丁 + 组同步切换）待下批 |

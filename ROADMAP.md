...
## 当前承诺（2026-07-12，唯一优先级声明）

优先级只在本节声明一次，下列各节只承载明细，不再各自宣称"当前/最高"：

1. **商用门板穿插队列**（P#8 → P#5 → P#2 → P#7，队内顺序为死约束，
   全部关闭前不得后延出队）
2. **PLCopen 合规补齐**（P1-A 已完成 → L2a 矩阵重写 → P5-B / P4-B1…）
3. **L 系列语言层**（ST-L0/ST-L1a 已交付；下一批次 ST-L1b / ST-L2）

硬件触发项到位则插队（台架下单 → F 轨 EtherCAT；真机在手 → H1 修订稿
送批）。PLCopen 扎实化里程碑已收口归档（见下节）。
...
| 5a | P-Part4 剩余 FB | 管理组矩阵 + 路径表/变换第二批矩阵均已批准实现 | ⚠️ Part 4 v2.0 权威清单为 **68 个 FB**；P4-B1 后当前 **40 个同名门面**、28 项无同名入口，另保留 2 个旧 Position 回读兼容门面。接口/语义仍逐项部分覆盖，不宣称合规，见 [完整条款审计](doc/compliance/plcopen-part4-clause-audit.md) |
...
## L 系列语言层（进行中，2026-07-11 维护者拍板启动）
...
当前进度：**ST-L0 与 ST-L1a 均已交付**（KB-069/070，矩阵均 2026-07-11
批准并同日实现收口——ST-L0：容错前端 + 确定性 VM + 50 黄金程序 +
10 万 fuzz；ST-L1a：16 标量宇宙 + 无损加宽白名单 + 210 格转换矩阵机读化
三方比对 + 双锚点哈希门）。下一批次候选：ST-L1b（复合类型）矩阵待起草 /
ST-L2（POU 与 MC_* 绑定）。扎实化收口项全部关闭（AxisGroup 第 1 批、executor 双域
...
排入 L 系列审批空窗（队内顺序与"不得后延出队"死约束以"当前承诺"
节的唯一声明为准）：

| 批 | 内容 | 量级 | 关的门 |
|----|------|------|--------|
| **P#8** | STO/SS1 集成边界文档（不做什么/边界线/集成商责任/CiA402·STO 交互口径） | 0.1 L0 | **完成（2026-07-13）**：指标 #8 的诚实集成边界全闭；STO/SS1 实现与认证仍硬门控 |
| P#5 | 分支覆盖首测 → 补盲区 → workflow 加 85% 分支门（2026-07-14 阶段收口：生产运动栈 73.4% → 78.4%，固定范围报告已接入，仍差约 460 条；达标时启用硬门，原 0.2 L0 估算不足） | 重新估算中 | 指标 #5 余项，未关闭 |
...
*最后更新：2026-07-12（ST-L0/ST-L1a 交付 + executor 双域 + 计划补遗盘点 + 扎实化归档、当前承诺单一化）*

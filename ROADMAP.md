...
| 4b | **信号通道并行项**（拷问后拉入：学习不等建造） | T1 pip wheel + 最小文档站起步——细案见 [signal-channel-plan](doc/planning/signal-channel-plan.md) | **pip wheel 已交付**（pyproject.toml + scikit-build-core + cibuildwheel CI + smoke test；本地构建验证通过）；文档站待后续 |

...
| **← 现在** | 2026-07-05 | 姿态批次已收口（KB-042）；等 v1.0.0-alpha 人工发布与硬件阶段触发 |
...
| L5 axis | 单轴全命令生命周期、组共享路径（2-8 轴）、前瞻窗口执行、坐标系栈（ACS/MCS/PCS + 工件帧/工具偏置）、kinematics 级联（龙门/SCARA）、位姿管线（RPY + 6R，TCP 工具变换）、双空间限速、B9 流会话 | KB-035/036/037/041/042 |
...
- 测试：34 目标全绿（oracle/fuzz/回放/分配卫兵/基准分层）；core 行覆盖 ≥85%

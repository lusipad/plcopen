...
**D3 在线调试示波器已完成（2026-07-24，KB-091）**：
...
保持独立。[PR #23](https://github.com/lusipad/plcopen/pull/23) 已合并，
主线 [Windows](https://github.com/lusipad/plcopen/actions/runs/30071051259)、
...
**D1 ST Language Server 已完成并合入（2026-07-24，KB-092）**：
...
均已落地；[PR #26](https://github.com/lusipad/plcopen/pull/26) 合入后的
...
主线门禁全绿，[中文](https://lusipad.com/plcopen/guides/st-language-server/)
和[英文](https://lusipad.com/plcopen/en/guides/st-language-server/)指南已上线。
**H1 同步关节流已完成并合入（2026-07-25，KB-035）**：
`JointStreamGroup` 已扩为 48 关节固定容量原子混合帧，direct/upsample、
整帧预检、keep-latest、本地周期 watchdog、组级断流、混合字段安全斜坡
和命令快照均有专项测试；`tau_ff` 仍只形成数据通路，不代表 T18 或真机
完成。[PR #29](https://github.com/lusipad/plcopen/pull/29) 合入后的
...
**H3 固定基座动力学前馈已完成并合入（2026-07-26，KB-094）**：
`dyn::FixedBaseChain` 以固定容量 O(n) RNEA 覆盖 1～8 关节转动串联链，
使用 SI 秒制并在失败时保持输出逐位不变；单摆/2R 闭式、独立 ABA 往返和
势能梯度 oracle 均已通过。Windows Debug 91/91 非 fuzz + 11/11 fuzz，
Release 6×8 合计 48 关节本地实测 `5.050 µs/cycle`（硬门 10 µs），
RT scan 32 文件、install/find_package 与 FetchContent 均通过。
[PR #35](https://github.com/lusipad/plcopen/pull/35) 与合入后的
...
[Twin](https://github.com/lusipad/plcopen/actions/runs/30197177072) 和
[Documentation](https://github.com/lusipad/plcopen/actions/runs/30197177048)
主线门均全绿。H3 不消费 H1 `tau_ff`，T18、浮动基座、接触/全身动力学、
Python/MuJoCo 控制器和真机安全仍未授权。
...
结构三段：**阶梯 L0-L7**（依赖只向下）、**支撑库 kin/stream/dyn**（阶梯旁、
依赖只向内；kin/stream 被 L5 消费，dyn 由调用方持有）、**外圈消费面 st / L7 adapters**（两条平行的纯
...
| L5 axis | 单轴全命令生命周期、组共享路径（2-8 轴）、前瞻窗口执行、坐标系栈（ACS/MCS/PCS + 工件帧/工具偏置）、kinematics 插件接入（消费支撑库 kin：龙门/SCARA；两条平行互斥契约，非级联）、位姿管线（RPY + 6R，TCP 工具变换）、笛卡尔/位姿回读（含 RPY 反演万向节约定）、段内笛卡尔插补（直线/圆弧/blending + 前瞻窗口，逐周期逆解 + 测地姿态，opt-in；腕奇异可穿越）、窗口深度可配、双空间限速、B9 流会话（消费支撑库 stream） | KB-035/036/037/041~050 |
...
| **H3 固定基座动力学前馈** | **已完成并合入（2026-07-26，KB-094）**：1～8 关节 RNEA、严格模型/输入校验、输出原子性、单摆/2R/独立 ABA/势能 oracle、Release 48 关节 5.050µs、RT scan 32 文件与两类消费者均通过；PR #35 与合入后五条主线工作流全绿。只返回纯 `tau_ff`，不声明 T18、浮动基座、接触、全身跨链、真机或安全完成 |

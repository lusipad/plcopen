...
| **Z1～Z5 软件极致收口** | **仓库内已完成（2026-07-22）**：可执行 notebook；轴/组 SI 配置及 Python 绑定；Python/C++/ST 三条同源旅程与 Doxygen API；Conan/vcpkg 双消费者资产；全量 ErrorCode 结构化诊断和 trace HTML/SVG。Windows Debug 98/98、Conan 两消费者、vcpkg 严格 port、MkDocs strict、Doxygen/Graphviz、RT scan 与 18/2409 replay 均通过。合入后的 `Cold User` 公开入口复验及外部中央 registry 收录分别按 [冷用户登记](doc/planning/cold-user-test-log.md) 与外部维护者流程跟踪 |

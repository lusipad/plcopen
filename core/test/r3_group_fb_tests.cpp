...
int check_group_reset()
{
    ...
    if (!reset.outputs.done || reset.outputs.error ||
        group.status() != axis::GroupStatus::standby || x.status() != axis::AxisStatus::standstill)
    {
...
int check_group_read_status_and_positions()
{
    ...
    fb::FbGroupReadPosition actual;
    actual.group_ref = &group;
    actual.enable = true;
    actual.source = axis::GroupValueSource::actual;
    actual.call();
    if (!actual.valid || actual.position.size != 2 || !near(actual.position.value[0], 2.0, 1e-9) ||
        !near(actual.position.value[1], 4.0, 1e-9))
    {
        return fail("group read actual position");
    }

    fb::FbGroupReadPosition command;
    command.group_ref = &group;
    command.enable = true;
    command.source = axis::GroupValueSource::commanded;
    command.call();
    if (!command.valid || command.position.size != 2 ||
        !near(command.position.value[0], 2.0, 1e-9) || !near(command.position.value[1], 4.0, 1e-9))
    {
...
class CountOnlyKinematics final : public kin::Kinematics
{
  public:
    ...
    std::size_t joint_count() const override { return joints_; }

    std::size_t cartesian_count() const override { return cartesian_; }
...
{
  public:
    explicit CountOnlyPoseKinematics(std::size_t joints) : joints_(joints) {}
...
int check_group_configuration_rejections_are_atomic()
{
    ...
    group.add_axis(y);
    group.enable();
    if (group.set_workpiece_frame_rpy(1.0, 2.0, 3.0, 0.1, 0.2, 0.3) != rt::ErrorCode::ok ||
        group.set_tool_transform_rpy(4.0, 5.0, 6.0, 0.4, 0.5, 0.6) != rt::ErrorCode::ok ||
...
int check_group_parameter_and_dynamics_validation()
{
    ...
    for (int field = 0; field < 2; ++field)
    {
        ...
        if (field == 0)
            invalid.value[1].minimum = nan;
        if (field == 1)
            invalid.value[1].maximum = nan;
...
int check_group_enable_rejections_are_atomic()
{
    ...
    if (empty.enable() != rt::ErrorCode::ok || empty.enable() != rt::ErrorCode::invalid_argument ||
...
int check_group_ownership_rejections_are_atomic()
{
    ...
    if (owner.add_axis(owned) != rt::ErrorCode::ok ||
        contender.add_axis(owned) != rt::ErrorCode::out_of_range || owner.member_count() != 1 ||
        contender.member_count() != 0 || owned.group_owner() != &owner)
    {
        return fail("foreign owner add rejection is atomic");
    }
    axis::AxisModel absent;
    if (owner.remove_axis(absent) != rt::ErrorCode::out_of_range || owner.member_count() != 1 ||
        owned.group_owner() != &owner || absent.group_owner() != nullptr)
    {
...
int check_kinematics_contract_rejections_are_atomic()
{
    ...
    const kin::Kinematics *invalid[] = {&wrong_joints, &mismatched_spaces, &too_few_cartesian,
                                        &too_many_cartesian};
    ...
    if (group.set_pose_kinematics(&six_joint_pose, 0.0, 1.0) != rt::ErrorCode::invalid_argument ||
        !same_group_snapshot(before, snapshot_group(group)) ||
        group.set_pose_kinematics(&wrong_joint_pose, 0.0, 1.0) != rt::ErrorCode::invalid_argument ||
        !same_group_snapshot(before, snapshot_group(group)))
    {
...
int check_pose_kinematics_contract_rejections_are_atomic()
{
    ...
    CountOnlyPoseKinematics wrong_joint_pose(5);
    if (group.set_pose_kinematics(&wrong_joint_pose, 0.0, 1.0) != rt::ErrorCode::invalid_argument ||
...
int check_direct_motion_rejections_are_atomic()
{
    ...
    if (group.set_group_override(0.5) != rt::ErrorCode::unsupported ||
        ...
        !same_group_snapshot(before, snapshot_group(group)) || !group.direct_motion_active())
    {
...
int check_group_submit_rejections_are_atomic()
{
    ...
    for (int scenario = 0; scenario < 11; ++scenario)
    {
        ...
        case 3:
            ...
            break;
        case 4:
            command.acceleration = nan;
            break;
        case 5:
            command.deceleration = 0.0;
            break;
        case 6:
            command.deceleration = nan;
            break;
        case 7:
            command.jerk = 0.0;
            break;
        case 8:
            ...
            break;
        case 9:
            command.target.value[0] = nan;
            break;
        default:
            command.target.value[1] = nan;
            break;
        }
        const rt::Result<std::uint32_t> rejected = group.submit_linear(command);
        if (rejected || rejected.error() != rt::ErrorCode::invalid_argument ||
    ...
    for (int scenario = 0; scenario < 15; ++scenario)
    {
        ...
        case 11:
            ...
            break;
        case 12:
            command.aux.value[0] = nan;
            break;
        case 13:
            command.aux.value[1] = nan;
            break;
        default:
        ...
        const rt::ErrorCode expected =
            scenario == 14 ? rt::ErrorCode::unsupported : rt::ErrorCode::invalid_argument;
...
int check_group_motion_rejects_active_standalone_member()
{
    ...
    fb::FbStepReferencePulse search;
    search.axis_ref = &x;
    search.reference_signal.input = 0;
    search.execute = true;
    search.call();
    const std::uint32_t search_id = x.snapshot().active_command_id;
    if (search.outputs.error || !search.outputs.busy || !search.outputs.active || search_id == 0 ||
        x.status() != axis::AxisStatus::homing || group.status() != axis::GroupStatus::standby)
    {
    ...
    if (linear_result || linear_result.error() != rt::ErrorCode::invalid_argument ||
        group.status() != axis::GroupStatus::standby ||
        x.snapshot().active_command_id != search_id || x.status() != axis::AxisStatus::homing ||
    ...
    x.cycle();
    search.call();
    if (x.snapshot().command_position <= before || search.outputs.error || !search.outputs.busy ||
        !search.outputs.active)
    {
    ...
    offset_group.enable();
    const rt::Result<std::uint32_t> offset = offset_x.submit_superimposed(1.0, 0.1, 0.1, 0.1, 0.1);
    ...
    if (offset_linear || offset_circular ||
        ...
        offset_group.status() != axis::GroupStatus::standby || !offset_x.superimposed_active() ||
...
int check_active_group_rejects_member_replan()
{
    ...
    axes[1].cycle();
    group.cycle();
    if (!near(axes[0].snapshot().command_position, axes[1].snapshot().command_position, 1e-12) ||
        group.status() != axis::GroupStatus::moving)
    {
...
int check_group_fb_error_paths()
{
    ...
    // FbGroupReadPosition(actual): enable=false, null group
    {
        fb::FbGroupReadPosition pos;
        pos.enable = false;
        pos.source = axis::GroupValueSource::actual;
        pos.call();
    ...
    // FbGroupReadPosition(command): enable=false, null group
    {
        fb::FbGroupReadPosition pos;
        pos.enable = false;
        pos.source = axis::GroupValueSource::commanded;
        pos.call();
        if (pos.valid || pos.error)
        {
...
int check_group_configuration_fb_matrix()
{
    ...
    if (!write_parameter.outputs.error)
        return fail("write parameter null group");
    ...
    if (!write_reference.outputs.error)
        return fail("write reference null group");

    fb::FbGroupWriteDefaultDynamics write_default;
    write_default.execute = true;
    write_default.call();
    if (!write_default.outputs.error)
        return fail("write default null group");

    fb::FbGroupWriteJoggingDynamics write_jogging;
    write_jogging.execute = true;
    write_jogging.call();
    if (!write_jogging.outputs.error)
        return fail("write jogging null group");

    fb::FbGroupWriteSWLimits write_limits;
    write_limits.execute = true;
    write_limits.call();
    if (!write_limits.outputs.error)
        return fail("write limits null group");

    fb::FbGroupReadReferenceDynamics read_reference;
    read_reference.enable = true;
    read_reference.call();
    if (!read_reference.error)
        return fail("read reference null group");
    ...
    read_jogging.call();
    if (!read_jogging.valid)
        return fail("read jogging dynamics");
...
int check_group_readback_fb_matrix()
{
    ...
    if (!configuration.error)
        return fail("group configuration null group");
    ...
    group.enable();
    motion.call();
    if (!motion.valid || !motion.standstill)
        return fail("group motion state standby");
    ...
    if (!command_info.error)
        return fail("group command info unknown id");
    return 0;
}
...
int main()
{
    if (check_group_configuration_rejections_are_atomic() != 0 ||
        ...
        check_group_reset() != 0 || check_group_read_status_and_positions() != 0 ||

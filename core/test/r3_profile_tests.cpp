...
int check_position_profile_step_direct_takeover()
{
    ...
    if (!profile.outputs.command_accepted || !profile.outputs.busy ||
        axis.snapshot().active_command_id != first_id || !axis.command_pending(first_id + 1) ||
...
int check_position_profile_duration_scaled_to_zero_rejection()
{
    ...
    if (!profile.outputs.error || profile.outputs.error_id != rt::ErrorCode::invalid_argument ||
        ...
        axis.snapshot().active_command_id != 0 || axis.snapshot().last_completed_command_id != 0 ||
...
int check_position_profile_invalid_later_segment_rejected_atomically()
{
    ...
    const TestCase cases[] = {
        {InvalidField::target, rt::ErrorCode::invalid_argument,
         "position profile later NaN target"},
        {InvalidField::soft_limit, rt::ErrorCode::out_of_range,
         "position profile later soft-limit target"},
        {InvalidField::velocity, rt::ErrorCode::invalid_argument,
         "position profile later invalid velocity"},
        {InvalidField::acceleration, rt::ErrorCode::invalid_argument,
         "position profile later invalid acceleration"},
        {InvalidField::deceleration, rt::ErrorCode::invalid_argument,
         "position profile later invalid deceleration"},
        {InvalidField::jerk, rt::ErrorCode::invalid_argument,
         "position profile later invalid jerk"},
    };
...
int check_profile_scaled_duration_rejections()
{
    ...
    velocity.call();
    if (!velocity.outputs.error || velocity.outputs.error_id != rt::ErrorCode::invalid_argument ||
    ...
    const axis::AxisSnapshot acceleration_shortened_before = acceleration_shortened_axis.snapshot();
...
int check_profile_header_validation_matrix()
{
    ...
    for (int condition = 0; condition < 9; ++condition)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            invalid.acceleration = nan;
            break;
        case 3:
            invalid.acceleration = 0.0;
            break;
        case 4:
            invalid.deceleration = nan;
            break;
        case 5:

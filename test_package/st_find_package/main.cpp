...
{
    std::cerr << "st guide smoke: " << stage << " failed: "
              << rt::to_string(error) << '\n';
    return 1;
}
...
{
    std::cerr << "st guide smoke: scan failed at cycle " << cycle << ": "
              << st::to_string(error) << '\n';
    return 1;
}
...
int main()
{
    ...
    if(instance.bind_axis("AxisX", &axis) != st::BindingError::ok) {
        std::cerr << "st guide smoke: bind_axis failed\n";
        return 1;
    }
    ...
    if(powered_index < 0 || busy_index < 0 || done_index < 0 ||
       error_index < 0 || error_id_index < 0) {
        std::cerr << "st guide smoke: demo symbols not found\n";
        return 1;
    }
    ...
    for(; cycle < 4000; ++cycle) {
        ...
        saw_powered = saw_powered ||
                      instance.value_bool(static_cast<std::size_t>(powered_index));
        if(instance.value_bool(static_cast<std::size_t>(error_index))) {
            ...
            std::cerr << "st guide smoke: motion FB error: "
                      << rt::to_string(code) << '\n';
            return 1;
        }
    ...
    std::cout << "st guide smoke: powered=" << saw_powered
              << ", done=" << done
              << ", busy="
              << instance.value_bool(static_cast<std::size_t>(busy_index))
              << ", cycles=" << cycle

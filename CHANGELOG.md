...
- Add the cycle-time-efficiency trend metric to the core benchmark (`otg_duration_vs_baseline` in `PATH_METRICS`, long-term-plan 6.5) and draft the Part 4 circular-motion spec matrix (`doc/compliance/plcopen-motion-part4-circular-matrix-draft.md`, A3 — spec-first draft awaiting human approval, no implementation started).

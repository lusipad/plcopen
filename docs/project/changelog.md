<title>Changelog</title>

# Changelog

Use the changelog for repository-level change history and release records for
the installable state of a specific version.

## Where to look

| Need | Document |
|---|---|
| Unreleased repository changes | [CHANGELOG.md](https://github.com/lusipad/plcopen/blob/main/CHANGELOG.md) |
| Install, artifacts, evidence, and limits for v0.20.0 | [v0.20.0 release record](../releases/v0.20.0.md) |
| Published source artifacts | [GitHub Releases](https://github.com/lusipad/plcopen/releases) |
| Published Python packages | [PyPI project history](https://pypi.org/project/pyplcopen/#history) |

The
[canonical changelog](https://github.com/lusipad/plcopen/blob/main/CHANGELOG.md)
records notable changes on the repository timeline. A changelog entry does
not prove that a tag, GitHub Release, or registry package has been published;
those states must be checked independently.

## Versioning note

The project is on a pre-1.0 version line. Read each release record and its
known limits before upgrading, especially when relying on real-time behavior,
ST language coverage, or hardware adapters.

...
static int route_verify_and_shift(Segment *segments, double end_vel, double pos_offset);

static bool check_results(const Segment *segments, int length);

static void route_discretization(Segment *segments, int length, uint32_t frequency, double &prev_t_remain);

static int tiny_segment_merge(Segment *segments, int length, uint32_t frequency, int32_t &new_start_tick);
...
void print_all(Segment *segments, int num)
{
    for (int i = 0; i < num; ++i)
    {
        printf("segment %d: "
               ...
               "acc=%lf, "
               "t=%lf\n",
               i, segments[i].start_position, segments[i].end_position, segments[i].start_vel, segments[i].acc,
               segments[i].t);
    }
}
                          ...
                          double acc, double dec)
{
...
#ifdef MC_DEBUG
    printf("get a cmd: pos %lf -> %lf, vel %lf -> %lf -> %lf, acc: %lf, dec: %lf\n", start_position, end_position,
           start_vel, vel, end_vel, acc, dec);
#endif
    ...
    data.number_segment = route_verify_and_shift(data.segments, end_vel, start_position);
    ...
    data.number_segment = tiny_segment_merge(data.segments, data.number_segment, frequency, data.current_tick);
                           ...
                           double &end_vel)
{
    ...
    if (isOpposite(shift_tmp, start_vel))
    {
        ...
        set_route_segment(segments, 0.0, shift_pre, start_vel, (shift_pre < 0) ? dec : -dec, t);
    ...
    if (is_acc_neg(start_vel, end_vel_tmp))
    {
        acc2 = -acc2;
    }
    ...
    {
        v_uni = sqrt(
            ((2 * shift_tmp + (start_vel * start_vel / acc1) - (end_vel_tmp * end_vel_tmp / acc2)) * (acc1 * acc2)) /
            (acc2 - acc1));
    ...
    set_route_segment(segments + 1, shift_tmp, shift_pre + s_acc1, start_vel, acc1, t_acc1);

    shift_tmp = shift_pre + s_acc1;

    set_route_segment(segments + 2, shift_tmp, shift_tmp + s_uni, v_uni, 0.0, t_uni);

    shift_tmp += s_uni;

    set_route_segment(segments + 3, shift_tmp, shift_tmp + s_acc2, v_uni, acc2, t_acc2);

    shift_tmp += s_acc2;

    set_route_segment(segments + 4, shift_tmp, shift, end_vel_tmp, acc3, t_acc3);
...
static bool check_results(const Segment *segments, int length)
{
    ...
    {
        if (!std::isfinite(segments[i].start_position) || std::isnan(segments[i].end_position) ||
            !std::isfinite(segments[i].start_vel) || !std::isfinite(segments[i].acc))
            return false;
    }
...
static void route_discretization(Segment *segments, int length, uint32_t frequency, double &prev_t_remain)
{
    ...
    for (int i = 0; i < length; ++i)
    {
        ...
        segments[i].start_position +=
            segments[i].start_vel * each_t_remain[i] + segments[i].acc * each_t_remain[i] * each_t_remain[i] / 2;

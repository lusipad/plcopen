...
> 状态：**已完成（2026-07-25，PR #29，main `220e80c`）**。维护者以
> “按照顺序开发吧”确认上一轮明确审批项；本文收敛
> [轨迹流语义矩阵](../compliance/trajectory-stream-semantics.md) v2 修订稿中
> 相互冲突的“同拍响应 / 慢解分拍”与“命令快照 / 实际反馈”口径，作为
> H1 实现与验收基线。
...
| 缓冲 | 完整帧 keep-latest；同一 `cycle()` 前可接受多个严格递增帧，下一拍只呈现最后一帧，不做未来帧调度队列 | 高 | 上层需要按未来时间戳排程 waypoint |
| 时间域 | producer 时间戳只做严格单调排序和回显；生效拍、延迟和断流年龄使用 group 本地 cycle，未来时间戳不得延迟 watchdog | 高 | executor 提供经批准的跨主机周期锁相映射 |
...
| keep-latest 与本地时间 | 同拍前多帧只呈现最后一帧；任意未来 producer 时间戳不改变本地 dropout 拍 |

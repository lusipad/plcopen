...
**R4 软件闭环状态（2026-07-05 更新）**：默认 CMake 包目标、install/export、
`find_package`、`FetchContent`、demo 和 `pyplcopen` smoke 已切到新核 `core/`；
旧 `src/` 默认隔离到 `PLCOPEN_BUILD_LEGACY=ON`。R4 变更已按风险分级拆 commit
合入 `main` 并推送，迁移指南落地
[doc/migration-v0-to-v1.md](../migration-v0-to-v1.md)。本地证据见

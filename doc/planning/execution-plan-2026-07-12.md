# 执行计划清单（2026-07-12 建账；2026-07-21 重基线）

> 性质：**操作层执行清单**。优先级唯一事实源是 [ROADMAP「当前承诺」节](../../ROADMAP.md)，
> 本文件是其任务级展开 + 勾选账；批次细节以各链接的规格/拆解文档为准，
> 此处不复述。完成一项勾一项；顺序要变，先改 ROADMAP 再改这里。

## 0. 分工地图

| 执行者 | 负责线 | 现状 |
|--------|--------|------|
| Claude | 商用门板队列（§1）+ S2 Feetech（§2）+ 送批线（§3）+ 全仓口径守护 | 本清单主体 |
| Codex | PLCopen 合规实现线：L2a-Bind → P5-B → P4-B1（§4） | L2a-Spec 已交付（f94a25c） |
| 维护者 | 人专属清单（§5） | 4 项待办 |

## 1. 线①：商用门板穿插队列（死约束顺序 P#8 → P#5 → P#2 → P#7）

### P#8 STO/SS1 集成边界文档（0.1 L0）— 关商用指标 #8

- [x] 核对 CiA402 状态机与 STO/SS1 交互的官方事实（IEC 61800-5-2 §4.2.3/§5.5-5.6/§8，不抄正文；2026-07-13）
- [x] 写边界文档：我们不做什么 / 安全边界线 / 集成商责任矩阵 / CiA402·STO 交互口径
- [x] 与 VISION Safety 硬门控口径对齐（不宣称任何 SF 能力）；docs-sync
- [x] 门禁 + 提交（P#8 文档批；不关闭 S-08 实现缺口）

### P#5 分支覆盖首测 + 85% 分支门（0.2 L0）— 关商用指标 #5 余项

- [x] gcovr 分支覆盖首测（WSL/Linux CI 口径），产出盲区清单（2026-07-13，见 `doc/compliance/branch-coverage-baseline.md`）
- [x] 补盲区测试（公开 API 合同补测；生产运动栈 6004/7062 = 85.006%）
- [x] workflow 加 85% 分支门（固定范围 `--fail-under-branch 85`）
- [x] 门禁完成；本地改动待维护者决定提交

### P#2 精度证据套件（0.5 L0）— 关商用指标 #2 软件面

- [x] C2 五次 Bezier/圆弧稳速波动断言 < 0.1%（0.0775% / 约 7.6e-12%）
- [x] cam 相位误差断言 < 1 周期（0 拍）
- [x] blending 精度汇总断言（偏差 0.05 ≤ 公差 0.05，非零过角速度）
- [x] 产出 `commercial-gate-evidence.md` 八项证据总账（P#7 后当前 4/8）
- [x] KB-075 声明变更 + 黄金语料升级；本地门禁完成后待维护者决定提交

### P#7 文档四件套 + 运维手册（1 L0）— 前置：Codex 线 L2a-Bind 交付 ST 素材

- [x] 全 FB 参考 / 实时集成指南 / 调优指南 / TwinCAT·CODESYS 迁移指南
- [x] 运维手册：fault 码 → 原因 → 处置（含现象首查项与事故记录字段）
- [x] 与 Z3′（文档站内容）合流；`mkdocs build --strict` 通过

## 2. 线②：S2 Feetech STS adapter（0.3 L0，穿插①执行；纯软件可闭环）

按已批矩阵 [feetech-sts-adapter-semantics.md](../compliance/feetech-sts-adapter-semantics.md) 验收指标 4.1-4.8：

- [x] 测试先行：黄金字节用例 ≥12（4.1）+ 符号-幅值编解码边界表（4.2）（KB-074，2026-07-13）
- [x] `core/adapters/feetech.h` 协议纯函数层：帧编解码 / 总线调度 / 显式单位比例，零 IO（决策点 2.1）
- [x] FeetechSim 寄存器映像 + write→帧→sim→帧→read 全链路往返（4.3）
- [x] 解析器 fuzz 10 万输入零 crash（4.4）+ 调度预算静态断言 N=16/31 @50/100Hz、N=32 协议长度拒绝（4.5）+ 失联 M 周期时序（4.6）
- [x] RT 扫描纳入 feetech.h、scan 路径零分配（4.7）；门禁 + 提交
- [ ] **待核项关卡（4.8）**：Goal_Velocity/Acceleration 换算系数 + Status 错误位定义按官方手册锁定并记录进矩阵——锁定前不进真机（S5）里程碑

## 3. 线③：送批线（人批空窗正好执行线①）

- [x] ST-L1b1 枚举 + 子范围语义矩阵起草 → 待维护者批准（实现不在本批）
- [x] ST-L1b 实现：枚举/子范围、聚合、字符串与日期族均已闭合
- [x] ST-L2c / Bind Complete：134/134 FB、1476/1476 pins 全绑定，
  strict generator、typed registry、逐周期等价与零分配门通过（2026-07-17）
- [x] ST-L3/L4/L5/L6/L7 与 L∀：进程映像、标准函数、任务、SFC、调试面及
  feature-set `pending=0` 完成态门通过（2026-07-18）

## 4. Codex 线观察点（不动手，只核对）

- [x] L2a-Bind：AXIS_REF + 首批 10 个 MC_* ST 绑定——只消费 `plcopen-motion-part1-io.yml` 单一事实源（KB-071，2026-07-13）
- [x] P5-B（Part 5 缺失 6 项）：按已批准矩阵交付 StepBlock → DistanceCoded → HomeAbsolute → Flying+AbortPassive（KB-072，2026-07-13）；11/11 有门面但仍不宣称合规
- [x] P4-B1（管理与回读 19 项）：按已批准矩阵交付（KB-073，2026-07-13）；Part 4 40/68 有同名门面、28 项缺失，仍不宣称合规
- [x] P4-B2（工具/载荷 8 项 + 点动 2 项）：按已批准矩阵交付（KB-076，2026-07-15）；Part 4 50/68 有同名门面、18 项缺失，仍不宣称合规
- [x] L2a-Bind 落地后全仓口径同步检查：canonical 口径（门面 43/43、B 级 22/43 审计时点、D 项 4 关 16 开、不宣称合规）未漂移（2026-07-13）

> 后续收口：C4/KB-079 已关闭 D-01～D-20；C5/KB-080 已关闭 Part 5
> 11/11 软件合同；C6/KB-081 已完成 Beckhoff 核心能力对等总账。以上观察点
> 保留为 2026-07-12 执行快照，不再作为当前状态。

## 5. 人专属清单（阻塞在维护者；杠杆排序引自[主计划](master-plan.md)）

| # | 事项 | 阻塞什么 | 杠杆 |
|---|------|----------|------|
| 1 | 台架决策 | F 轨 EtherCAT、商用指标 #1/#4 与 S1 | ★★★ |
| 2 | ADR-0006 许可证核验 | F 轨仓库边界、主站选型与分发成本 | ★★ |
| 3 | SO-ARM101 双臂套件下单（+2 备用 STS3215） | S5 真机里程碑、R 系列（真硬件锚） | ★★★ |
| 5 | PLCopen 会员确认邮件（AI 起草、人发送） | 合规声明提交路径 | ★★ |

PyPI publisher 已由维护者注册；发布 tag / GitHub Release 不再列为人专属，
维护者明确指定版本与目标提交后可由 AI 执行。

硬件触发即插队：**真机在手 → H1 修订稿送批；台架下单 → F 轨 EtherCAT 插队**（优先于全部软件线）。

## 6. 当前队列与后备批次

- **P0（已完成）**：`17932de` 已把 `st_l0_runtime_tests.cpp` 的无谓按值复制
  改为只读引用；Windows/Linux 远端通过，包括 Clang 81 文件门、GCC 与 ARM64。
- **P1（已完成）**：`0195739` 已把 11 项 fuzz 从日常 CTest/coverage 分流到
  原生 Nightly 并远端复验，ARM/QEMU 测试 28:40 → 4:57；`7336379` 在保持
  81 TU 与完整检查项的前提下将 `clang-tidy` 改为 8 路并行，远端 E2
  22:17 → 10:31，Linux 总时长 27:59 → 16:46。
- **A1（已完成）**：`805a363` 交付[浮点数值语义合同](../design/core/floating-point-semantics.md)、
  consumer 严格编译选项与跨平台运行值测试；Windows/Linux/ARM64 与三平台
  Wheels 远端复验通过。
- **A2（已完成，2026-07-20）**：T30 WCET 度量工具已交付版本锚定成本表、无分配报告与环境化 Release 观测；不声称 certified WCET。
- **G1（已完成，2026-07-21）**：贡献流程、单维护者决策/继任声明、安全报告与 ST 不可信输入资源边界已公开；私密漏洞报告和 org 迁移保留为人侧动作。
- **E5（软件实现完成，远端证据待闭环，2026-07-21）**：四类指标、同机
  base/head 9 对门、二次确认、严格解析与 90 天 JSON 历史已接入 Linux
  workflow；本地自对拍通过，尚待首次 bootstrap 和随后一次真实远端比较。
- **X5（软件实现完成，远端证据待闭环，2026-07-21）**：固定 ABI Servo
  setpoint/feedback SPSC + 状态双缓冲、owner 映射期不可转让、整帧输入校验、
  Windows/Linux 两进程对拍与 Linux TSan 本地通过；两平台 Nightly 首轮待推送。
- **AxisGroup 第 1-5 批（已完成，2026-07-21）**：connector、joint look-ahead、
  Cartesian path/window、frame/pose/kinematics 与 MoveDirect lifecycle/path 已按
  行为簇形成明确 owner，group.h 7744→4487 行；93/93、RT scan、18/2409 replay
  与 find_package/FetchContent consumer 全绿。共享调度边界按设计保留。
- **条件插队**：ADR-0006 许可证核验与台架决策完成后，F1→F2→F3。
- Y/P/Z/E 四线候选见[软件极致计划](software-excellence-plan.md)
- OOP 裁决、OPC UA 触发条件修正——[合规计划人专属项](plcopen-conformance-plan.md)，随复盘

## 7. 刷新规则

- 优先级变更：先改 ROADMAP「当前承诺」，再改本清单——不允许两处打架
- 每批收口：勾选本清单 + STATUS.md 同步（既有惯例）
- 硬件价格观察哨随[价格观察表](hardware-price-watch.md)季度刷新（下次 2026-10）

---

*创建：2026-07-12；2026-07-19 按 L 系列闭合与当前队列重基线。会话任务账镜像：#9/#11/#13-#18。*

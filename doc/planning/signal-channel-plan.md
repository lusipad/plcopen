...
>
> **2026-07-22 收口**：Z1 已补齐可执行 notebook；Z3 的三条主旅程最终
> 采用 Python / C++ / ST，算法白盒降为深入参考。选择 ST 是因为语言层已
> 闭合且 `ST → MC_* → AxisModel` 已成为独立消费入口，不再只是算法导读。
...
- 版本：跟随内核 SemVer（当前正式版本为 0.20.0；1.0 等待 ABI、硬件与
  采纳证据成熟后再讨论）；
- 内容物：现有 AxisSim/PoseArmSim/流接口/cam 工具全量 + README 快速
  开始（中文为主 + 英文摘要节）；
- 发布：PyPI Trusted Publisher 与 GitHub `pypi` environment 已配置；
  `v0.20.0` 已由 tag 工作流通过 OIDC 发布，后续版本沿用同一流程；
...
- v1 结构（只做门廊，不搬运全部文档）：
  ...
  - 三条 30 分钟旅程：Python 数字孪生（pip 起步）/ C++ 嵌入
    （find_package 起步）/ ST 直驱 MC（编译、绑定、周期 scan）；
  - 算法白盒、矩阵与 oracle 导读保留为参考入口，不占新用户主路径；

...
| single-axis management | R3.5 | `core/axis/state.h` plus `core/fb/motion.h` power, reset, readback snapshot, override, torque, limits | `plcopen_core_r3_tests`; Part 1/2 matrix rows remain authoritative |
| single-axis motion | R3.6 | `AxisModel::submit` and L6 facades for absolute, relative, velocity, home, halt, stop, torque | `plcopen_core_r3_tests`; R1 OTG tests; R2 sampler tests |
| multi-axis sync | R3.7 | `core/exec/sync.h` gear/cam primitives plus synchronized axis state in `AxisGroup` | `plcopen_core_r2_tests`; `plcopen_core_r3_tests` |

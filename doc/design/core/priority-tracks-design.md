...
| `core/dyn/`（**已实现，KB-094**） | L2 级纯数学 | H3 | **分链前馈/重力补偿**——每条 1～8 关节固定基座转动链独立 RNEA（重力/科氏/惯量前馈；腿/臂/腰由调用方分链，ADR-0005 口径）；参数结构体（质量/质心/完整惯量）、独立 ABA/势能 oracle（仅测试层）；**全身浮动基座 + 接触动力学显式非目标**（上层职责）；无 PLCopen 语义，进 RT 扫描 | ✅ [H3 v1 矩阵](../../compliance/dynamics-feedforward-semantics.md)实现候选与本地门已完成；PR/main 远端证据待闭合 |
...
| 动力学前馈（**分链**） | 全部固定基座链 RNEA 求和 ≤10µs/周期（≈48 关节分布于各链）；浮动基座/接触项非目标 | H3 本地 Release 6×8 实测 5.050µs（KB-094） |
...
T15 dyn/RNEA（已完成）→ T18 扭矩流矩阵 →（扭矩通道实现）
T17 基座帧（小，随需插入）
```

- 状态标注（2026-07-26）：**T24/H1 已实现**（KB-064/035）、**H2
  已实现**（KB-089；7DOF 止于 core/offline，L5 seam 仍六轴），T2a 已完成
  （KB-090），T2b 也已完成并通过主线/公网证据（KB-093），**T15/H3
  实现候选与本地证据完成**（KB-094；远端证据待闭合）；

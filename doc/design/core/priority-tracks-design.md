...
| `python/` + `tools/`（扩展） | 工具面 | T2a/T2b | pip wheel（cibuildwheel）、双关节 feedback 闭环、七关节 H1 q/dq 原子帧闭环、Rerun 记录与单位换算辅助 | KB-090 已完成；KB-093 候选实现完成，待远端/主线证据 |
...
H2 serial_chain（已完成）──────────────┼─→ T2a/T2b rerun 孪生（T2b 候选）
T1 pip wheel（已完成）─────────────────┘

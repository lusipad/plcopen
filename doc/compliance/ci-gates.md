...
|------|----------|------|----------|------------------|
| Windows 主线 | `.github/workflows/windows-ci.yml` | `push`、`pull_request`、`workflow_dispatch` | RT scan、Part 1/2 与 Part 5 I/O matrix、replay fixture、完整 CTest（72 项，含 Part 5 C5、ST conformance 锚点及转换矩阵三方比对）、Windows coverage artifact（延迟门基准豁免插桩重跑，预算门仍在 CTest 强制）、installed/fetchcontent consumer | 是 |
| Linux 主线 | `.github/workflows/linux-ci.yml` | `push`、`pull_request`、`workflow_dispatch` | gcc/clang Release 构建、RT scan、Part 1/2 matrix、replay fixture、完整 CTest（含 ST conformance 锚点、两个字节码锚点哈希及转换矩阵跨平台复验）、benchmark baseline、clang-tidy gate、ARM64/QEMU（matrix_check 经 CMAKE_CROSSCOMPILING_EMULATOR 执行）、Python binding、consumer smoke、API docs 生成 | 是 |
...
| Core Nightly | `.github/workflows/core-nightly.yml` | 每日 `02:23 UTC`、`workflow_dispatch` | **五个独立 job**：1,000,000 轮 deterministic OTG fuzz、A2 allocation 50,000,000-cycle soak、1,000,000 轮 time-optimal OTG fuzz、executor 双域 TSAN（20,000 周期 @4kHz，ADR-0007）、ST 单 PROGRAM 与 L2b 多 POU 编译/有限执行 fuzz 各 100,000 输入（固定静态运行时缓冲，ASan/UBSan）；单项失败不跳过其余项 | 周期质量门；不替代主线 CI |

...
| Coverage Gate | `.github/workflows/coverage.yml` | 每周一 `03:47 UTC`、`workflow_dispatch` | `cmake/coverage_gate.cmake` + gcovr 8.6，core line coverage >= 90%（下一次定时跑首次覆盖 st 语言层面，若跌破按模块补盲区） | 周期质量门；发布前按需手动确认 |

...
| D-09 | TorqueControl 建立持续 command owner；普通运动接管清 torque owner，InTorque 持续比较 |
| D-10 | AccelerationProfile 按时间-加速度段积分，完成后保持积分终速 |
| D-11 | immediate SetPosition 可运动中无扰动重标定；Relative 以 actual position 为基准 |
...
| TorqueControl 无真实驱动反馈 | InTorque 比较 commanded torque；不宣称实际电流/扭矩到达 |
...
- 新增命令错误固定账本、Stop 锁、Torque owner、加速度 profile 与统一
  Enable 回读生命周期，均保持固定容量周期路径；

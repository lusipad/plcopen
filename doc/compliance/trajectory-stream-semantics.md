...
### 修订追加（T23/T25 待批准；T24 已实现、默认关闭）

| # | 决策点 | 提案 |
|---|--------|------|
| 5 | 快路径（T24，设计见 [stream-fastpath-design](../design/core/stream-fastpath-design.md)） | 事件重解可选走**定时五次多项式闭式快解**；速度/加速度/jerk 用解析极值校验，位置包络在归一化时间域验证完整段内部，禁止固定点采样充当安全证明；越限或数值不可证即落完整求解链。剖面按 Horner 周期采样；配置默认关闭，既有回放保持关闭态口径，显式启用由 fast-path oracle 与流测试护栏 |
...
修订版验收追加：快路径命中率作为恒速/正弦/滤波噪声三类 fuzz 的观测
趋势；硬门为快慢两态终态等价 ≤1e-9、最坏全慢拍 <300µs，且 48 关节
分批重解最迟 5 拍收敛。衰减律与增益斜坡逐周期断言、direct 模式包络
钳位与看门狗语义、帧基准双口径（稳态/对抗拍）入 STREAM_METRICS。

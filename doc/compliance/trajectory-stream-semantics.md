...
- **求解节奏**：事件驱动——新目标与滑行漂移各触发一次求解，锁定滑行
  零规划；完全相同的静止目标只刷新 watchdog，不重启相同剖面；断流外推期
  每周期合成目标求解（非常态路径）。
- **停车回退加固（2026-07-31）**：反向 quintic 的包络证明以段内保存的
  精确边界速度代替多项式端点重算，避免浮点尘埃伪造反向；普通求解链全部
  无法给出可证明候选时，`stopping` 独占的最终回退使用加速度归零恒 jerk
  段与 exact ramp-to-zero，并再次经过动力学和完整位置包络证明。H1 六关节
  反例覆盖重复静止帧、反向目标和组级断流，终态全零且无 filter fault；
  `core-stream-session` 等既有黄金回放逐位不变。

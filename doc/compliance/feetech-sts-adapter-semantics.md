...
> 状态：**草案，待维护者批准（2026-07-12）**。本文件是 H1 载体计划
...
| 2.2 | 目标机型与协议 | STS/SMS 系列（协议 0）；首发 STS3215（型号号 777，4096 counts/圈，磁编码绝对单圈）。SCS 系列（协议 1）不做 | H1/SO-ARM101 实机就是它 |
...
| 2.6 | 换算系数待核项 | Goal_Velocity 与 Acceleration 的寄存器单位换算系数**以官方手册数值锁定后才可进实现**（本矩阵不编造数字）；锁定记录补进本文件实现记录节 | 不确定的事实标待核，不猜 |
...
| 2.11 | 舵机侧保护位 | Status 错误位映射两级：过温/过流/过载 → drive fault（进 ErrorStop 路径）；电压越界 → warning。**具体位定义以官方手册核对后锁定（同 2.6 待核项）** | 保护策略归内核/用户，adapter 只如实翻译 |
...
| 2.14 | 容量档位 | 定长支持 N ≤ 32 舵机/总线；构造期超限拒绝 | H1 需 16；32 覆盖双臂+腿冗余 |
...
| 4.5 | 调度预算静态断言：N=16/32 @50/100Hz 帧字节数与周期预算表一致 | 编译期/测试期断言 |

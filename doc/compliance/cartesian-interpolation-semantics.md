...
## v2 增补实现记录（2026-07-06，KB-045/KB-047）

v2-A 腕奇异通过、v2-B 笛卡尔圆弧、v2-C 笛卡尔 blending（KB-049）
全部落库。v2-C 为 KB-031 风格整链单剖面（拐角限速作用全链——急拐角
常正确降级，结点级限速属笛卡尔窗口批次）。收敛声明：带阈 1e-8（较批准案 1e-6 收紧，保 1e-8 线性度
门槛）；奇异重定向拍以 step 门为界（0.5 预算不覆盖零测度区间跳变）；
弧平面 = 笛卡尔 XY、z 沿参数线性（KB-030 约定移植，空间弧平面留
后续）。验收 `plcopen_core_cartesian_tests` 全绿。
...
## v3 增补：笛卡尔前瞻窗口（已批准，2026-07-06）

解除 KB-049 的整链限速代价：连续笛卡尔 blending 后继构成**笛卡尔域
前瞻窗口**——直线段不再被最急拐角拖慢，急拐角轮廓不再整段输给停车
基线。KB-032 的窗口机器复用于笛卡尔坐标域。

| # | 决策点 | 提案 | 理由 |
|---|--------|------|------|
| 1 | 适用面 | **v1 仅平移插件组**（SCARA/龙门类，`set_kinematics`）；位姿组（6R）维持 KB-049 单后继链，窗口化留 v2（姿态跨拐角语义需专门设计） | 轮廓加工速度收益集中在平移构型；不为 v1 偷渡姿态语义 |
| 2 | 窗口构成 | 连续 `blending_low/high` + `mcTMMaxCornerDeviation` 笛卡尔直线后继加入窗口（容量 = `set_window_depth` 配置值）；几何存**插件笛卡尔空间**（直线段 + 五次 Bezier 拐角，KB-031 原语） | 复用已验证的窗口/拐角机器 |
| 3 | 结点限速 | 结点速度 = min(两命令限速, 拐角曲率限速 √(a/κ), 双向 jerk 精确可达扫描（KB-039 机器）, `cartesian_velocity_limit`)；每段独立 jerk-limited 剖面按结点速度衔接 | KB-032/039 口径逐字移植到笛卡尔度量 |
| 4 | 逐周期执行 | 窗口采样得笛卡尔点 → 解析逆解（seed = 上周期关节，KB-044 机器）→ 成员 setpoint；逆解/margin 失败 = 组 errorstop（KB-044 决策 #7） | 周期路径 = 窗口采样 + 一次逆解 |
| 5 | 扩展预验证 | 新段 + 拐角沿弧长 33 采样 seed 链逆解 + margin 检查（从前驱端点关节续链）；失败拒绝扩展（`infeasible`/`precondition_failed`），已承诺窗口不受影响 | KB-044 预验证口径 |
| 6 | 降级与拒绝 | 反折、过迟、容量、不优于停车基线 → 按 KB-032 既有降级/报告口径；joint↔cartesian 混模窗口、窗口内圆弧、位姿组窗口 → `unsupported` | 零静默 |
| 7 | GroupStop | 沿已承诺窗口几何复合弧长受控停车（KB-032 机器直接作用） | 免费获得 |
| 8 | 回放 | 新黄金场景 `core-group-cartesian-window`（SCARA 多段折线）；默认与既有路径逐位不变 | 周期路径新增执行分支必须有黄金护栏 |

验收指标：密集折线节拍显著优于停车基线（口径同 KB-032：≤80%）且
优于 KB-049 单链（急拐角场景不再降级）；每周期 TCP 距窗口几何
≤1e-8；逐周期关节步有界；扩展/降级/拒绝表全显式；既有语料逐位不变。

不做（v3 范围外）：位姿组窗口、窗口内笛卡尔圆弧、弧-线笛卡尔过渡、
跨段姿态测地。

**v3 实现记录（2026-07-06，KB-050）**：平行迷你机器落库（不触碰关节
窗口状态机，零回归）。收敛声明三处：① 匀速内段与拐角匀速骑行（终拍
≤1 周期量化）——结点钉在命令限速上时 OTG vt≠0 巡航精化无可行区间，
量化残差落入深潜燃烧（实测 0.07 距离 34 拍），骑行口径构造性消除；
② 活体入口段出口速度退避梯子（1.0/0.99/0.97/0.94，衔接量化步均在
包络内）；③ v1 容量 16 段/32 件（组对象内存与嵌入面权衡；测试台
必须 static 的教训随组对象增肥再次生效）。验收：6 段缓拐折线实测
优于 0.8× 停车基线门槛、90° 拐角结点限速通过不降级。

...
|------|---------|-----------|---------|--------|--------|------|
| Part 1 | v2.0 (2011) | **43** | **43 个门面** | 名称面 100%；C4 已关闭 D-01～D-20，正式声明仍不完整 | "45/45" | 🟡 KB-079 关闭核心语义缺陷；B/E/V 与认证材料仍不能由数量替代 |
| Part 4 | v2.0 (2026-05) | **68** | **68 个同名门面** | 名称面 100%，条款不合规 | "零留白" | 🟡 C3 后门面齐全，但 B/E/O 接口与语义仍多为部分覆盖 |
| Part 5 | v2.0 (2011) | **11** | **11 个标准门面；45 B + 102 E 已逐项声明** | 软件合同 11/11；正式合规未提交 | "已交付" | 🟡 C5（KB-080）关闭名称、派生类型、I/O 与可软件验证语义；硬件真实性和批准仍开放 |
...
> [Part 4 条款审计](plcopen-part4-clause-audit.md)。C6 已删除旧名 Position
> 回读包装，只保留 v2 `MC_GroupReadPosition(Source)`。
...
C5（KB-080）已补齐并复核全部 11 个标准 FB，逐 I/O 事实源为
`plcopen-motion-part5-io.yml`，生成结果为
`generated/plcopen-motion-part5-io.md`。当前剩余项是 IEC 基础类型替代、
真机堵转/编码器/时间戳/TorqueLimit 证明及供应商签署批准，不再存在软件
门面或已定义语义缺口。
...
| 4 | P-Part5b/C5：补齐并收口 Part 5 11 个标准 FB | ✅ 已完成 | KB-080；45 B + 102 E 机读声明 |

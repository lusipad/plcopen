...
| 8 | §9.8 `MC_GroupHome` | ⚠️ | Position/CoordSystem/BufferMode 已进入真实组队列；Aborting 接管、Buffered 顺序及 GroupHoming 周期有测试；Position 仍是固定容量宿主结构 |
| 9 | §9.9 `MC_GroupPower` | ⚠️ | C3 Enable 电平控制全部成员并锁存组错误；缺 EnablePositive/Negative 与 MC_Power 双写仲裁 |
| 10 | §9.10.1 `MC_SetKinTransform` | ⚠️ | tagged `KinTransformRef` 明确 none/kinematics/pose；immediate/queued 均真实应用到组队列尾；仍是 C++ 插件非拥有引用 |
| 11 | §9.10.2 `MC_SetCartesianTransform` | ⚠️ | 六标量固定 6D RPY；CoordSystem 选择 PCS/TCS，ExecutionMode 支持 immediate/queued |
| 12 | §9.10.3 `MC_SetCoordinateTransform` | ⚠️ | C3 vendor ref 适配固定 Cartesian 6D PCS/TCS，支持 immediate/queued；非 Cartesian 不支持 |
| 13 | §9.10.4 `MC_ReadKinTransform` | ⚠️ | tagged `KinTransformRef` 回读平移/位姿插件非拥有引用；不是标准厂商数据结构 |
| 14 | §9.10.5 `MC_ReadCartesianTransform` | ⚠️ | CoordSystem 真实选择 PCS 工件帧或 TCS 工具帧，并输出统一 6D transform；接口仍非标准引用形态 |
| 15 | §9.10.6 `MC_ReadCoordinateTransform` | ⚠️ | C3 按 CoordSystem 回读固定 6D PCS 或 TCS RPY；其他坐标系不支持 |
...
| 25 | §9.20 `MC_GroupWriteParameter` | ⚠️ | immediate/queued 真实消费；DynamicsMode 支持 absolute/percentage，Transition 仅 end-point，start-point `unsupported` |
...
| # | 条款 / FB | 状态 | 当前证据或缺口 |
|---:|---|---|---|
| 34 | §9.33 `MC_GroupWriteSWLimits` | ⚠️ | immediate 或组队列尾 queued 整表原子写；限制后续最终 ACS 目标 |
| 35 | §9.25 `MC_GroupStop` | ✅ | `FbGroupStop`；受控停止和接管测试覆盖 |
| 36 | §9.26 `MC_GroupHalt` | ⚠️ | Aborting 原路径受控到零；Buffered 作为组队列屏障在前序运动后完成；新 Aborting 运动可中止 |
...
| 43 | §9.38 `MC_MoveDirectAbsolute` | ✅ | CoordSystem/Buffer/Transition 全进入 GroupCommand；独立 PTP 或显式协调 lookahead，见 KB-068 |
| 44 | §9.39 `MC_MoveDirectRelative` | ✅ | 同上；相对目标按活动/排队尾端确定解析 |
| 45 | §9.40 `MC_GroupWaitTime` | ✅ | Duration 为 ST TIME 纳秒并按 task period 向上量化；Aborting/Buffered 生命周期可观察 |
| 46 | §9.41 `MC_PathSelect` | ⚠️ | supplier-specific `PathDescription` 源与 `PathData` 选择结果已分离；固定 32 点、无堆、原子校验复制；仍是项目自定义数据布局 |
| 47 | §9.42 `MC_MovePath` | ⚠️ | PathData/CoordSystem/BufferMode/TransitionMode/TransitionParameter 已进入真实 submit/窗口语义；当前 PathData 仍只承载线性 waypoint |
| 48 | §9.43 `MC_GroupSetOverride` | ✅ | 标准 Enable/Enabled 电平合同与 Vel/Acc/Jerk 三因子已承接；越界夹紧，零速度因子语义见 KB-058/067 |
| 49 | §9.44 `MC_GroupJog` | ✅ | Vel/Acc override 实际缩放；TCP 线/角最大距离形成精确可观察边界 |
| 50 | §9.45 `MC_GroupJogVector` | ✅ | MCS/PCS 6D 向量连续更新，Vel/Acc override 实际缩放 |
...
| 55 | §9.50 `MC_GroupWriteToolData` | ⚠️ | 固定 16 槽、0 号恒等、原子写及 immediate/queued ExecutionMode 已实现 |
...
| GroupHome | AxesGroup→`group_ref` | Execute/Position→`execute/position` | CoordSystem/BufferMode→同名字段 | 六通用输出→通用输出 | ✅ 输入进入真实组队列；固定容量 Position 为宿主表示 |
| SetKinTransform | AxesGroup→`group_ref` | Execute/KinTransform→`execute/kin_transform` | ExecutionMode→`execution_mode` | Done/Busy/Error/ErrorID→通用输出 | ⚠️ tagged 引用与排队语义已承载；宿主仍扩展 margin/step |
| ReadCartesianTransform | AxesGroup→`group_ref` | Enable/CoordSystem→`enable/coord_system` | — | Valid/Error/ErrorID/六维 Transform→`valid/error/error_id/trans_* + rot_angle*` | ⚠️ PCS/TCS 选择真实承载；仍缺 Busy |
| GroupReadStatus | AxesGroup→`group_ref` | Enable→`enable` | — | Valid/Busy/Error/ErrorID + 状态位→`valid`/—/`error`/`error_id` + `group_homing` 等状态位 | ⚠️ 缺 Busy；增加 Interrupted |
...
| MoveLinearAbsolute | AxesGroup→`group_ref` | Execute/Position/Velocity/Acceleration/Deceleration/Jerk→同义字段 | CoordSystem/BufferMode/TransitionMode/TransitionVelocity/TransitionParameter/OrientationMode→同义字段 | Done/Busy/Active/CommandAborted/Error/ErrorID→通用输出 | ⚠️ 14-pin 字段族齐；Position 数据结构、模式值域与路径速度仍有偏差 |
| MoveLinearRelative | AxesGroup→`group_ref` | Execute→`execute`; Distance→继承 `position`; 动态→同名 | CoordSystem/Buffer/Transition/Orientation→同名 | 六通用输出→通用输出 | ⚠️ 原生语义齐；Distance 字段仍命名为 `position` |
| MoveCircularAbsolute | AxesGroup→`group_ref` | Execute/AuxPoint/EndPoint/PathChoice/动态→同义字段 | CircMode/CoordSystem/Buffer/Tolerance/Transition/Orientation→同义字段 | 六通用输出→通用输出 | ⚠️ 14-pin 字段族齐；CircMode 只承接 BORDER |
| MoveCircularRelative | AxesGroup→`group_ref` | Execute/AuxPoint/EndPoint/PathChoice/动态→同义字段 | CircMode/CoordSystem/Buffer/Tolerance/Transition/Orientation→同上 | 六通用输出→通用输出 | ⚠️ 原生语义齐；Relative 数据仍名 EndPoint |
...
| PathSelect | AxesGroup→`group_ref`; PathData→`path_data`; PathDescription→`path_description` | Execute→`execute` | — | Done/Busy/Error/ErrorID→`outputs.*` | ⚠️ supplier-specific 固定容量表示；源描述与选择结果已显式分离 |
| MovePath | AxesGroup→`group_ref`; PathData→`path_data` | Execute→`execute` | CoordSystem/BufferMode/TransitionMode/TransitionParameter→同名字段 | 六通用输出→通用输出 | ⚠️ 四输入均进入真实 planner；PathData 仍只支持线性 waypoint |
| GroupSetOverride | AxesGroup→`group_ref` | Enable→`enable`; Vel/Acc/JerkFactor→同名字段 | 三因子均实际进入组重规划限值 | Enabled/Busy/Error/ErrorID→同名字段 | ✅ 标准电平接口；VelFactor=0 暂停 |
...
| `MC_GroupHome` §9.8 | Group/Execute/Position/CoordSystem/BufferMode 可映射 | Aborting 接管、Buffered 排队；全成员写入目标位置后完成 | 执行前 GroupHoming 可读，完成回 Standby | 接受周期 Busy，执行周期后 Done | ⚠️ home_direct 为确定性单周期成员执行，不模拟驱动搜索过程 |
| `MC_SetKinTransform` §9.10.1 | `KinTransformRef` 以 tag 选择一种 C++ 插件引用；ExecutionMode 可映射 | queued 严格位于既有运动队列尾，非法 tag/指针组合原子拒绝 | 执行时保持 Standby | immediate 同周期 Done；queued 执行后 Done | ⚠️ 厂商插件类型仍是宿主扩展 |
| `MC_ReadCartesianTransform` §9.10.5 | Enable/CoordSystem/统一 6D transform 可映射 | PCS 选择工件帧，TCS 选择工具帧 | 不改变 | Enable 真即 Valid，假清零 | ⚠️ 无 Busy；宿主 ToolData 不是标准引用 |
| `MC_GroupReadStatus` §9.29 | Enable、GroupHoming 与主要状态位可映射 | 状态位持续反映组状态及待执行 Home | 不改变 | Enable 真即 Valid，假清除 | ⚠️ 增加 Interrupted；成员同步时折叠为 Moving |
...
| `MC_PathSelect` §9.41 | supplier-specific `PathDescription*` 输入和 `PathTable*` 结果可映射 | 全表校验后原子生成选择结果 | 不改变 | 当前同周期 Done | ⚠️ 32 点上限、进程内 selection handle 为项目扩展 |
| `MC_MovePath` §9.42 | PathData 与 CoordSystem/Buffer/Transition 四字段可映射 | 逐段坐标解析；首段队列接管与相邻过渡；后续窗口执行 | Standby/Moving→Moving→Standby | Busy/Active→Done | ⚠️ 只提交线性 waypoint，非完整路径元素 |
| `MC_GroupSetOverride` §9.43 | Enable + Vel/Acc/JerkFactor 完整 | 逐周期修改组三项动态比例，速度零值暂停 | Moving 保持 Moving；零值驻留 | 同周期应用后 Enabled；处理期 Busy | ✅；零值恢复修复见 KB-058/067 |
...
| 变换 Set/Read | `MC_SetCartesianTransform`、`MC_SetCoordinateTransform`、`MC_ReadKinTransform`、`MC_ReadCoordinateTransform` | vendor ref 适配为 tagged 插件引用和固定 6D RPY；PCS/TCS immediate/queued 已承接，非 Cartesian ref 不支持 |
...
| 8 | `MC_SetKinTransform` | 保留 | ⚠️ tagged C++ 插件引用 |
| 9 | `MC_SetCartesianTransform` | 保留 | ⚠️ 固定 6D PCS/TCS，immediate/queued |
| 10 | `MC_SetCoordinateTransform` | 保留 | ⚠️ Cartesian 6D vendor ref 子集 |
| 11 | `MC_ReadKinTransform` | 保留 | ⚠️ tagged 插件引用回读 |
| 12 | `MC_ReadCartesianTransform` | 保留 | ⚠️ 六标量 PCS/TCS 回读 |
| 13 | `MC_ReadCoordinateTransform` | 保留 | ⚠️ 固定 6D PCS/TCS 回读 |
...
| D4-10 | 中 | 字段审计剩余：Direct 缺 CoordSystem，多项读 FB 缺 Busy；Circular Transition、PathSelect/MovePath 数据与模式字段、GroupHome 与 Override 已闭合 | 继续按本文 §5.1 的具体缺字段补接口合同测试 |
...
| D4-13 | 已关闭 | `MC_GroupHome` 已进入组命令队列，接受周期 Busy/GroupHoming，执行完成后 Done；Aborting/Buffered 顺序有专项测试 | 保持组回零命令账本与单周期 home_direct 边界声明 |

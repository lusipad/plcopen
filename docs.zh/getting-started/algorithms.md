...
跃度输入 `u(t)` 会在 `{+j, 0, -j}` 之间切换，因此最多形成 7 个分段。生产求解器
**不**枚举切换结构：它以闭式构造「入口 ramp → 巡航 → 出口 ramp」轮廓，选取距离
单调的分支，对巡航速度做二分以命中目标距离，把每个相位下取整到整数周期，余量交
给五次段（quintic）吸收。随后由候选级联挑出最短可行解：取整多相位、精确多相位、
精修巡航、最小单段五次，以及兜底基线。

Pontryagin 切换结构枚举只存在于测试侧，作为独立的最优性 oracle
